...
class PlotRepository:
    ...
    def wiggle(self, unique_filename, sample_positions=None, trace_positions=None, color='black', stretch_factor=0.15):
        ...
        # Input check
        traces, sample_positions, trace_positions, trace_spacing = self._wiggle_input_pre_processing(
            traces, sample_positions, trace_positions, stretch_factor)
        number_of_traces = traces.shape[1]

        plot = figure(
            x_range=(trace_positions[0] - trace_spacing,
                     trace_positions[-1] + trace_spacing),
